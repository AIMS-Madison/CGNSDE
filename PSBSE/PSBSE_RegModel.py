...
torch.save(regmodel.state_dict(), r"/home/cc/CodeProjects/CGNSDE/NonCG/NonCG_Model/NonCG_regmodel.pt")
np.save(r"/home/cc/CodeProjects/CGNSDE/NonCG/NonCG_Model/NonCG_regmodel_train_loss.npy", train_loss_history)

# regmodel.load_state_dict(torch.load("/home/cc/CodeProjects/CGNSDE/NonCG/NonCG_Model/NonCG_regmodel.pt"))
...
# Data Assimilation
with torch.no_grad():
    mu_preds, R_preds = CGFilter(regmodel, u1=test_u[:, [0]].reshape(-1, 1, 1), mu0=torch.zeros(2, 1).to(device), R0=0.01*torch.eye(2).to(device), cut_point=0, sigma_lst=sigma_hat)
...
axs[0].set_title(r"\textbf{RegModel}", fontsize=35)
...
np.random.seed(0)
with torch.no_grad():
    u_longSimu = SDESolver(regmodel, test_u[0], steps=500000, dt=0.001, sigma_lst=sigma_hat)[::10]
...
ax20.set_ylim( [min(np.min(test_u[:,2]),np.min(u_longSimu[:,2]))-1, max(np.max(test_u[:,2]), np.max(u_longSimu[:,2]))] )
...
ax20.set_yticks([-4, -2, 0, 2])
ax01.set_xticks([-1.5, 0, 1.5])
ax11.set_xticks([-4, -2, 0])
ax21.set_xticks([-4, -2, 0, 2])
